...
export const getMHResources = async (resourceType) => {
  const querySnapshot = db
    .collection("mentalHealthResources")
    .doc(resourceType)
    .get()
    .then(function (result) {
...
class JournalEntry {
  ...
  toString() {
    return (
      ...
      this.status +
      ", " +
      this.t2eEntryMoodAnalysis +
      ", " +
      this.t2eSentMoodAnalysis +
      ", " +
      this.polarityEntryMoodAnalysis +
      ", " +
...
const entryConverter = {
  ...
  fromFirestore: (snapshot, options) => {
    ...
    return new JournalEntry(
      ...
      data.createdAt,
      data.status,
      data.t2eEntryMoodAnalysis,
      data.t2eSentMoodAnalysis,
      data.polarityEntryMoodAnalysis,
...
export const searchByDate = async (date) => {
  ...
  const q = query(
    ...
    where("createdAt", "<", upperLimit),
    where("createdAt", ">=", millis)
  );
